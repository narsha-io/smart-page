...
@EnableWebMvc
@SpringBootTest(classes = {SmartPageSpringTestApplication.class, WebConfig.class}, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class ResolverTest {
    ...
    public void noFilterWithPageAndSort() {
        assertThat(call("?page=10&size=300&sort=id,asc&sort=firstName,desc").getStatusCode()).isEqualTo(HttpStatus.OK);
        final var query = reference.get();
    ...
    public void noFilterWithPageAndUnknownSort() {
        assertThat(call("?page=10&size=300&sort=id,asc&sort=test,desc").getStatusCode()).isEqualTo(HttpStatus.OK);
        final var query = reference.get();
        assertPageAndTarget(query, 10, 300);
    ...
    public void validFilters() {
        assertThat(call("?id=1&firstName=toto&filter=id,equals&filter=firstName,equals").getStatusCode()).isEqualTo(HttpStatus.OK);
        final var query = reference.get();
        assertPageAndTarget(query);
    ...
    public void withDefaultFilterAndUnknownFilter() {
        assertThat(call("?id=1&firstName=toto&filter=id,equals&filter=truc,equals").getStatusCode()).isEqualTo(HttpStatus.OK);
        final var query = reference.get();
        assertPageAndTarget(query);
    ...
    private ResponseEntity<String> call(String param) {
        return this.restTemplate.getForEntity("http://localhost:" + port + "/test" + (param == null ? "" : param), String.class);
    }
    ...
    private void assertPageAndTarget(PaginatedFilteredQuery<Person> query, Integer page, Integer size) {
