...
public class JpaQueryExecutor implements QueryExecutor {
    ...
    public <T> Pair<List<T>, Long> execute(PaginatedFilteredQuery<T> paginatedFilteredQuery, RowMapper rowMapper) {
        final NativeQuery query = this.entityManager.createNativeQuery("select id, name from countries")
    ...
    private <T> List<T> extractData(NativeQuery query, PaginatedFilteredQuery<T> paginatedFilteredQuery, RowMapper rowMapper) {
